...
function Sidebar() {
    ...
    return (
        <div className="flex h-[calc(100vh-3.5rem)] min-w-[220px] flex-col border-r-[1px] border-r-richblack-700 bg-richblack-800 py-10">
            ...
            <NavLink to={settinglink.path} className={`relative px-8 py-2 text-sm font-medium ${settinglink.path === locaation.pathname
                ? "bg-yellow-800 text-yellow-50"
                : "bg-opacity-0 text-richblack-300"
                } transition-all duration-200`}>
                <span
                    className={`absolute left-0 top-0 h-full w-[0.15rem] bg-yellow-50 ${settinglink.path === locaation.pathname ? "opacity-100" : "opacity-0"
                        }`}
                ></span>
